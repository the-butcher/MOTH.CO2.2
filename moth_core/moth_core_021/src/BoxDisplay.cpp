...
const Rectangle RECT_TOP = {1, 1, 295, 22};
const Rectangle RECT_BOT = {1, 106, 295, 127};
const Rectangle RECT_CO2 = {1, 22, 207, 106};
const Rectangle RECT_DEG = {207, 22, 295, 64};
const Rectangle RECT_HUM = {207, 64, 295, 106};
const Rectangle RECT_BAT = {RECT_BOT.xmax - 16, RECT_BOT.ymin + 6, RECT_BOT.xmax - 7, RECT_BOT.ymin + 19};
...
void BoxDisplay::updateConfiguration() {
    ...
    thresholdsTemperature = {degRiskLo, degWarnLo, degWarnHi, degRiskHi};
    thresholdsHumidity = {humRiskLo, humWarnLo, humWarnHi, humRiskHi};
    thresholdsHighlight = {0, 0, 10, 20};
...
ValuesCo2 BoxDisplay::getDisplayValuesCo2() {
    ...
    if (cor > 0 && Measurements::memBufferIndx > valueCount) {
        ...
        if (temperatureAvg > temperature) {
            ...
            return {latestMeasurement.valuesCo2.co2, temperature, humidity};
        } else {
...
void BoxDisplay::renderFooter() {
    ...
    // percentage
    BoxDisplay::fillRectangle({RECT_BAT.xmin, RECT_BAT.ymax - 3 - (int)round((RECT_BAT.ymax - RECT_BAT.ymin - 3) * valuesBat.percent * 0.01), RECT_BAT.xmax, RECT_BAT.ymax}, EPD_BLACK);

    // battery contact clip
    baseDisplay.drawFastHLine(RECT_BAT.xmin + 1, RECT_BAT.ymin + 1, 2, EPD_WHITE);
    baseDisplay.drawFastHLine(RECT_BAT.xmax - 3, RECT_BAT.ymin + 1, 2, EPD_WHITE);
