...
String BoxConn::VNUM = SensorPmsa003i::ACTIVE ? "1.0.p13.pms" : "1.0.p13";
    ...
    if (BoxFiles::existsPath(BoxConn::CONFIG_PATH)) {
        ...
        if (fileSuccess) {
            ...
            if (root.success()) {
                // timeout minutes
                _wifiTimeoutMinutes =
                    root[JSON_KEY___MINUTES] | _wifiTimeoutMinutes;

                int configuredNetworkIndex = 0;
                int networkCount =
                    root[JSON_KEY__NETWORKS].as<JsonArray>().size();
                for (int i = 0; i < networkCount; i++) {
                    String key =
                        root[JSON_KEY__NETWORKS][i][JSON_KEY_______KEY] | "";
                    String pwd =
                        root[JSON_KEY__NETWORKS][i][JSON_KEY_______PWD] | "";
                    if (pwd != "") {
                        pwd = BoxEncr::decrypt(pwd);
                    }
                    configuredNetworks[configuredNetworkIndex] = {
                        key, pwd, 0, WIFI_AUTH_OPEN};
...
bool BoxConn::isExpireable() {
    ...
    return mode != WIFI_OFF &&
           millis() > wifiExpiryMillis;  // not off AND expirable
}
...
void BoxConn::begin() {
    ...
    server.on("/api/status", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        response->addHeader(
            "Cache-Control",
            "max-age=60");  // assume status does not change too quick
        ...
        dispJo["config"] =
            BoxConn::formatConfigStatus(BoxDisplay::configStatus);
    ...
    server.on("/api/latest", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        DateTime date =
            DateTime(SECONDS_FROM_1970_TO_2000 + latestMeasurement.secondstime);
        ...
        int maxAge =
            ...
            (BoxClock::getDate().secondstime() -
             latestMeasurement.secondstime);  // time until next measurement
        char maxAgeBuf[16];
    ...
    server.on("/api/data", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        int maxAge = 60 - (BoxClock::getDate().secondstime() -
                           Measurements::getLatestMeasurement()
                               .secondstime);  // time until next measurement
        char maxAgeBuf[16];
        sprintf(maxAgeBuf, "max-age=%s", String(maxAge));
        response->addHeader("Cache-Control", maxAgeBuf);
    ...
    server.on("/api/folder", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        response->addHeader(
            "Cache-Control",
            "max-age=10");  // assume no frequent changes in folder content
    ...
    server.on("/api/file", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        if (request->hasParam("file")) {
            ...
            if (dataFileName != BoxEncr::CONFIG_PATH &&
                BoxFiles::existsPath(dataFileName)) {  // hide encr from api

                File32Response *response =
                    new File32Response(dataFileName, "text/csv");

                int maxAge = 86400;  // 24 hours
                if (dataFileName == Measurements::dataFileNameCurr) {
                    maxAge = (Measurements::csvBufferSize -
                              Measurements::getCsvBufferIndex()) *
                             60;  // time until next file update
                }
                char maxAgeBuf[16];
                sprintf(maxAgeBuf, "max-age=%s", String(maxAge));
                response->addHeader("Cache-Control", maxAgeBuf);
    ...
    server.on("/api/encrypt", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        response->addHeader(
            "Cache-Control",
            "max-age=10");  // tell the browser to not call too often
        ...
        if (request->hasParam("value")) {
            String encrypted =
                BoxEncr::encrypt(request->getParam("value")->value());
            root[CODE] = 200;
            root["encr"] = encrypted;
    ...
    server.on("/api/networks", HTTP_GET, [](AsyncWebServerRequest *request) {
        ...
        response->addHeader(
            "Cache-Control",
            "max-age=10");  // networks are only evaluated when turning wifi
                            // off, then on again, ne frequent changes
        ...
        if (request->hasParam("ref")) {
            ...
            if (BoxConn::checkNumeric(refRaw)) {
                ...
                if (ref >= 400) {
                    ...
                    response->addHeader(
                        "Cache-Control",
                        "max-age=180");  // no sense in calibrating quicker than
                                         // 3 minutes
                } else {
                    root[CODE] = 400;
    ...
    server.onNotFound([](AsyncWebServerRequest *request) {
        ...
        if (BoxFiles::existsPath(url)) {
            ...
            response->addHeader(
                "Last-Modified",
                "Mon, 22 May 2023 00:00:00 GMT");  // TODO this should change
                                                   // with updates to the server
                                                   // files
            request->send(response);
    ...
    sprintf(networkConnBuf, "WIFI:T:WPA;S:mothbox%s;P:CO2@420PPM;;",
            String(_chipmacid, HEX));
    apNetworkConn = String(networkConnBuf);

    WiFi.onEvent(BoxConn::handleStationConnected,
                 WiFiEvent_t::ARDUINO_EVENT_WIFI_STA_CONNECTED);
}

void BoxConn::handleUpdate(AsyncWebServerRequest *request, String filename,
...
void BoxConn::handleUpload(AsyncWebServerRequest *request, String filename,
                           size_t index, uint8_t *data, size_t len,
                           bool final) {
        ...
        if (index == 0) {
            if (BoxFiles::existsPath(dataFileName) && index == 0) {
                ...
                if (lastIndexOfSlash > 0) {
                    String dataFilePath =
                        dataFileName.substring(0, lastIndexOfSlash);
...
void BoxConn::on() {
    stNetworkName = "";  // reset network, we dont know where we will end up
                         // being connected to
    ...
    for (int ssidIndex = 0; ssidIndex < ssidCount; ssidIndex++) {
        ...
        // prevent crash that would occur if an index outside the array size
        // were adressed
        if (ssidIndex < 10) {
            discoveredNetworks[ssidIndex] = {ssidCandidate, passCandidate,
                                             WiFi.RSSI(ssidIndex),
    ...
    for (int ssidIndex = ssidCount; ssidIndex < 10; ssidIndex++) {
        discoveredNetworks[ssidIndex] = {"", "", 0, WIFI_AUTH_OPEN};
    }

    // either no network found to connect to or no connection possible -> go
    // into ap mode
    if (WiFi.status() != WL_CONNECTED) {
        BoxConn::off();
        delay(500);
...
wifi_mode_t BoxConn::getMode() { return mode; }
