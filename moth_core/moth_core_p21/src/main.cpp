...
typedef enum {
    ...
    SENSORS_GETVALS   // sensors can provide values from measurements previously
                      // taken
} sensors_mode_t;
...
// TODO :: define functions for all buttons (maybe attachable to the button
// handlers) this could maybe create the possibility to move the
// handleButtonChangeA, ... methods to the button handlers could there be a
// typedef that holds both action AND display character for that action
...
void setup() {
    ...
    BoxFiles::begin();
    BoxClock::begin();
    BoxDisplay::begin();  // BoxFiles required to read config
    BoxPack::begin();
    BoxEncr::begin();  // BoxFiles required to read config
    BoxConn::begin();  // BoxFiles and BoxEncr required to read config
    BoxMqtt::begin();  // BoxFiles and BoxEncr required to read config
    Measurements::begin();

    BoxPack::tryRead();  // need to read to have value on splash

    BoxDisplay::renderMothInfo(BoxConn::VNUM);
    BoxDisplay::hibernate(true);  // must be true to maintain Serial
...
void renderState() {  // bool force
    ...
    if (autoConnect) {
        ...
        BoxClock::optNtpUpdate();  // will only update if
                                   // BoxClock::isUpdateable() is true, needs to
                                   // be called explicitly due to no
                                   // station_connected event
    }
    ...
    if (autoConnect) {
        BoxConn::off();  // if it was forced to be on, turn off directly
                         // afterwards
    }
...
void loop() {
    ...
    if (tryReadWaitSecondsA <= 1) {
        ...
        SensorScd041::tryRead();  // delays 5000 ms (no button press possible
                                  // during that time)
    ...
    // regardless of loopAction, a measurement will be taken if it is time to do
    // so however, there may be issues when this happens shortly before a
    // measurement ...
    // ... in such cases it can happen that i.e. turning on WiFi consumes enough
    // time to have a negative wait time to the next measurement
    int64_t measureWaitSecondsA = getMeasureWaitSeconds();
    if (measureWaitSecondsA <= 1) {
        BoxBeep::setPixelColor(COLOR_MAGENTA);

        // a final, short delay to hit the same second at all times, as far as
        // possible
        if (measureWaitSecondsA == 1) {
            delay(MILLISECONDS_PER_SECOND);
        }

        // if the PMS is read in the SCD41 and BME280 tryread section, it will
        // be 10 seconds early the fan may not have run long enough as of spec
        ...
        sensorsMode = SENSORS_TRYREAD;  // only after reading a measurement,
                                        // another read can be tried

        // beep when on an thresholds exceeded, measuremnt needs to be
        // re-fetched, because co2 gets altered by low-pass filter
    ...
    if (loopAction == LOOP_REASON______CALIBRRATION) {
        ...
        /**
         * assumption is that the last three measurement are used as average
         * reference value but with single shot this may be noisy, therefore the
         * average is subtracted from the requested value and the current low
         * pass filter value is added in an attempt for compensation
         */
        Measurement measurement;
        int lowPassValue;
        int avgHistValue = 0;
