...
const Adafruit_BME280::sensor_mode MODE_FORCED =
    (Adafruit_BME280::sensor_mode)0b01;
const Adafruit_BME280::sensor_sampling SAMPLING_X1 =
    (Adafruit_BME280::sensor_sampling)0b001;
const Adafruit_BME280::sensor_filter FILTER_OFF =
    (Adafruit_BME280::sensor_filter)0b000;
const Adafruit_BME280::standby_duration STANDBY_MS_1000 =
    (Adafruit_BME280::standby_duration)0b101;
...
void SensorBme280::begin() {
    ...
    SensorBme280::baseSensor.setSampling(
        MODE_FORCED, SAMPLING_X1, SAMPLING_X1, SAMPLING_X1, FILTER_OFF,
...
void SensorBme280::setTemperatureOffset(float temperatureOffset) {
    bool isApplOffsetRequired =
        temperatureOffset != SensorBme280::temperatureOffset;
...
void SensorBme280::setAltitudeOffset(float altitudeOffset) {
    ...
    SensorBme280::pressureOffset =
        0;  // triggers a recalculation upon next measurement
}
...
ValuesBme SensorBme280::getValues() {
    ...
    SensorBme280::values = {SensorBme280::baseSensor.readTemperature(),
                            SensorBme280::baseSensor.readHumidity(), pressure,
                            SensorBme280::getAltitude(pressure)};
    return SensorBme280::values;
}

void SensorBme280::updateAltitude(float altitudeUpdate) {
    SensorBme280::setAltitudeOffset(SensorBme280::values.altitude +
                                    altitudeUpdate);
...
void SensorBme280::applyTemperatureOffset() {
    SensorBme280::baseSensor.setTemperatureCompensation(
        -SensorBme280::temperatureOffset);
}

float SensorBme280::getAltitude(float pressure) {
    return (1 - pow(pressure * 0.01 / SensorBme280::pressureOffset,
                    ALTITUDE__EXP)) *
           ALTITUDE_MULT;
}
...
void SensorBme280::applyAltitudeOffset(float pressure) {
    SensorBme280::pressureOffset =
        pressure * 0.01 /
        pow(1 - SensorBme280::altitudeOffset / ALTITUDE_MULT,
            1 / ALTITUDE__EXP);
}
