...
void Measurements::begin() {
    ...
    if (SensorPmsa003i::ACTIVE) {
        Measurements::CSV_HEAD =
            "time; co2; co2_raw; temperature; humidity; temperature_bme; "
            "humidity_bme; pm010; pm025; pm100; pc003; pc005; pc010; pc025; "
            "pc050; pc100; pressure; percent\r\n";
        Measurements::CSV_FRMT =
            "%04d-%02d-%02d %02d:%02d:%02d; %s; %s; %s; %s; %s; %s; %s; %s; "
            "%s; %s; %s; %s; %s; %s; %s; %s; %s\r\n";
    }
}
...
void Measurements::saveToFile() {
    ...
    for (int offsetIndex = Measurements::csvBufferSize - 1; offsetIndex >= 0;
        ...
        if (dataFileDef.name != dataFileNameLast) {
            ...
            if (csvFile.size() == 0) {  // first time this file is being written
                                        // to -> write csv header
...
/**
 * on the latest measurement replace the bme values (i.e. for a change in
