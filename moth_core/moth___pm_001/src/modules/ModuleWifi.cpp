...
void ModuleWifi::configure(config_t& config) {
    ...
    ModuleCard::removeFile(WIFI_CONFIG__DAT);
    ModuleWifi::createDat(config);
}

void ModuleWifi::createDat(config_t& config) {

    File wifiFileJson = LittleFS.open(WIFI_CONFIG_JSON.c_str(), FILE_READ);
    if (wifiFileJson) {
        ...
        if (!error) {
            ...
            File wifiFileDat = LittleFS.open(WIFI_CONFIG__DAT.c_str(), FILE_WRITE);  // the file has been checked to not exist
            if (wifiFileDat) {
...
bool ModuleWifi::powerup(config_t& config, bool isManualActivation) {
    ...
    File wifiFileDat = LittleFS.open(WIFI_CONFIG__DAT.c_str(), FILE_READ);
    if (wifiFileDat) {
        network_t readValue;
