  ...
  useEffect(() => {
    ...
    loadDispConfig().then(_disp => {
      loadWifiConfig().then(_wifi => {
        loadMqttConfig().then(_mqtt => {
          if (_disp['code']) {
            console.warn('failed to load config/disp.json', _disp);
            _disp = DISP_CONFIG_DEFAULT;
          }
          if (_wifi['code']) {
            console.warn('failed to load config/wifi.json', _wifi);
            _wifi = WIFI_CONFIG_DEFAULT;
          }
          if (_mqtt['code']) {
            console.warn('failed to load config/mqtt.json', _wifi);
            _mqtt = MQTT_CONFIG_DEFAULT;
          }
          handleUpdate({
            disp: {
              ..._disp,
            ...
            mqtt: {
              ..._mqtt,
              cli: _mqtt.cli ? _mqtt.cli : `moth${boxUrl.substring(boxUrl.lastIndexOf('.')).replace('.', '_').replace('/api', '').padStart(4, '_')}`,
              min: _mqtt.min ? _mqtt.min : 5,
              status: 'LOADED_VALID'
            }
          });
        }).catch(e => {
    ...
    <>
      ...
      <Stack direction={'column'} sx={{ margin: '5px', width: '800px', ...props.style }}>
        ...
        <Stack direction={'column'} sx={{
          ...
          />
          <LabelledDivider label='PMS thresholds and settings' />
          <ConfigChoice
            caption='PMS warn level'
            value={{
              type: 'number',
              fixed: true,
              value: disp.pms.wHi,
              ...
              min: 1,
              max: disp.pms.rHi,
              handleUpdate: value => handleDispUpdate({ pms: { wHi: value } })
            }}
          />
          <ConfigChoice
            caption='PMS risk level'
            value={{
              type: 'number',
              fixed: true,
              value: disp.pms.rHi,
              ...
              min: disp.pms.wHi,
              max: 50,
              handleUpdate: value => handleDispUpdate({ pms: { rHi: value } })
            }}
          />
          <ConfigChoice
            caption='PMS low pass filter'
            value={{
              type: 'number',
              fixed: false,
              value: disp.pms.lpa,
              ...
              handleUpdate: value => handleDispUpdate({ pms: { lpa: value } })
            }}
          />
