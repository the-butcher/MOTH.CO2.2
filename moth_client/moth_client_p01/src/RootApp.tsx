...
const RootApp = () => {
  ...
  const boxUrl = `http://192.168.0.130/api`; // when running directly from device
  ...
  const handleConfigUpdate = (updates: Partial<ITabConfigProps>) => {
    ...
    if (updates.disp && !areObjectKeysEqual(updates.disp.pms, tabConfigPropsRef.current.disp.pms, ['wHi', 'rHi'])) {
      seriesDefUpdateRequired = true;
      (SERIES_DEFS.pm010.colorMap as IPiecewiseColorConfig).thresholds = [
        updates.disp.pms.wHi,
        updates.disp.pms.rHi
      ];
      (SERIES_DEFS.pm025.colorMap as IPiecewiseColorConfig).thresholds = [
        updates.disp.pms.wHi,
        updates.disp.pms.rHi
      ];
      (SERIES_DEFS.pm100.colorMap as IPiecewiseColorConfig).thresholds = [
        updates.disp.pms.wHi,
        updates.disp.pms.rHi
      ];
    }
  ...
  useEffect(() => {
    ...
    console.log(`⚙ updating root component (status)`, status);
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Stack direction={'row'} spacing={0} sx={{ height: '100%' }}>
        ...
        {
          status ? <>
            <TabValues key={'values'} {...tabValuesProps} style={{ display: viewType === 'values' ? 'flex' : 'none' }} />
